...
class MyTopo:
    ...
    def create_topo(self):
        ...
        c0 = self.net.addController('c0', ip='127.0.0.2')
        
        info('Adding Hosts \n')
        for i in range(0, len(fetched_hosts)):
            f_h = fetched_hosts[i]
            h = self.net.addHost(f'h{i+100}')#, mac=f_h['mac'], ip=f_h['ipv4'], ipv6=f_h['ipv6'])
            self.hosts.append(h)
            
        info('Adding Switches \n')
        for i in range(0, len(fetched_switches)):
            f_s = fetched_switches[i]
            s = self.net.addSwitch(f's{i+100}', protocols=self.SWITCH_PROTOCOL)#, dpid=f_s['switch_dpid'])
        ...
        for i in range(0, len(fetched_links)):
            ...
            if src_switch == -1 or dst_switch == -1:
                print(f"Error occured: switch idxs are {src_switch}, {dst_switch}")
    ...
    def get_switch_idx_by_id(self, switches, fetched_switches, dpid):
